...
/// File entry returned by listing endpoints.
...
/// Validate and resolve a path, ensuring it is under the base directory.
/// CRITICAL: This prevents path traversal attacks.
fn safe_resolve(base_dir: &str, relative_path: &str) -> Result<PathBuf, String> {
    let base = PathBuf::from(base_dir);

    // If the relative path is absolute, strip leading / so it becomes relative
    ...
    // If the requested path doesn't exist yet (for write/mkdir), we canonicalize
    // its parent and then append the final component.
...
/// Check if a file extension suggests it is a text file.
...
pub async fn list_files(
    ...
    let dir_path = match safe_resolve(&config.paths.base_dir, relative) {
        Ok(p) => p,
        Err(e) => {
    ...
    match std::fs::read_dir(&dir_path) {
        Ok(read_dir) => {
            for entry in read_dir.flatten() {
                ...
                // Build a relative path from the base dir
                let rel_path = path
                    .strip_prefix(&config.paths.base_dir)
...
pub async fn read_file(
    ...
    let file_path = match safe_resolve(&config.paths.base_dir, &query.path) {
        Ok(p) => p,
        Err(e) => {
            return HttpResponse::Forbidden().json(ErrorBody { error: e });
        }
...
pub async fn write_file(
    ...
    let file_path = match safe_resolve(&config.paths.base_dir, &body.path) {
        Ok(p) => p,
        Err(e) => {
            return HttpResponse::Forbidden().json(ErrorBody { error: e });
        }
...
pub async fn upload_file(
    ...
    while let Some(item) = payload.next().await {
        ...
        if field_name == "path" {
            // Read the target directory path
        ...
        if field_name == "file" {
            ...
            let target_path = match safe_resolve(
...
pub async fn mkdir(
    ...
    let dir_path = match safe_resolve(&config.paths.base_dir, &body.path) {
        Ok(p) => p,
        Err(e) => {
            return HttpResponse::Forbidden().json(ErrorBody { error: e });
        }
...
pub async fn delete_file(
    ...
    let target_path = match safe_resolve(&config.paths.base_dir, &query.path) {
        Ok(p) => p,
        Err(e) => {
            return HttpResponse::Forbidden().json(ErrorBody { error: e });
        }
    ...
    let canonical_base = PathBuf::from(&config.paths.base_dir)
        .canonicalize()
        .unwrap_or_else(|_| PathBuf::from(&config.paths.base_dir));
